...
rollback; -- 오류가 발생해서 초기화 ( 마지막 저장 단계 )
select * from dept01;
...
as select empno, ename, job, mgr, hiredate, emp.deptno, dname, loc
from emp, dept
where emp.deptno=dept.deptno
and emp.deptno=10
;
...
-- Data Dictionary 
-- view 정보 확인 : user_views 테이블을 통해 확인이 가능 로그인한 사용자의 정보
...
-- 인라인 뷰를 이용해서 가장 최근에 입사한 사원 5명을 추출해보자!
select *
from (select * from emp order by hiredate desc)
where rownum<=5; 


-- sequence : 숫자 자동 생성기, 시작값, 증가값, 최대값, 반환여부
...
-- creat index index_name on targer Table_name(culumn_)
