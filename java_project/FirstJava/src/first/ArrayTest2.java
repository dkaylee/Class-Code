...
public class ArrayTest2 {
	...
	public static void main(String[] args) {
		...
		System.out.println(arr.length);		//2차원 배열 arr의 legth 값은 1차원 배열의 개수를 가진다.
		...
		System.out.println("2차원 배열의 요소출력=====================");
		...
		System.out.println("2차원 배열의 데이터 저장===================");
		// 전체요소를 대입
		/*
		 * for(int i=0; i<arr.length; i++) { // 0,1,2 for(int j=0; j<arr[i].length; j++)
		 * { // 0,1,2,3 arr[i][j] = i+j; } }
		 */
		...
		/*
		 * for(int i=0; i<arr.length; i++) { // 0,1,2 for(int j=0; j<arr[i].length; j++)
		 * { // 0,1,2,3 System.out.print(arr[i][j]+"\t"); } System.out.println(); }
		 */
		...
		//for-each에서의 값의 변경 -> 배열과는 영향을 주지 않는다.
		for(int n : arr4) {
			n++;
			System.out.println(n);
		}
	...
	static void showData(String[][] arr) { //오버로딩
