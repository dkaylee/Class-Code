...
def crawling(place, place_lists):
    ...
    for i, place in enumerate(place_lists):
        ...
        #place_category = place.select('.head_itme > .tit_name > .subcategory')[0].text # place subcategory
        ...
        try:
            ...
            sleep(1)
            extract_detail(place_name) # 리뷰 추출
...
def extract_detail(place_name):
    ...
    # 첫 페이지 리뷰 목록 찾기
    detail_infos = soup.select('.details_placeinfo > div')

    # 리뷰가 있는 경우
    if len(detail_infos) != 0:
        for i, detail in enumerate(detail_infos):
            category = detail.select('.location_evaluation > span') #카테고리
            time = detail.select('.text_operation > span') # 시간
            contact = detail.select('.location_present > span.txt_contact') # 연락처
            val = ''
            if len(time) != 0:
                if len(contact) != 0:
                    val = time[0].text + '/' + contact[0].text.replace('점', '')
                else:
                    val = time[0].text + '/0'
                print(val)
